...
desc_1 = [
    ...
    (
        "柚子",
        "文旦柚產地價格(八里、冬山、西湖、斗六、麻豆、下營及瑞穗等農會查報價格之簡單平均)",
    ),
    ("蓮霧", "蓮霧產地價格(六龜、枋寮及南州等農會查報價格之簡單平均)"),
    (
        "鳳梨",
        "金鑽鳳梨產地價格(名間、古坑、民雄、關廟、大樹、高樹和內埔等農會查報價格之簡單平均)",
    ),
...
def get_avg_price(qs, has_volume, has_weight):
    if has_volume and has_weight:  # 新增有日均重量的品項計算平均價格公式
        ...
        return (
            total_price.sum() / total_volume_weight.sum()
            if len(total_volume_weight)
            else 0
        )
    elif has_volume:
...
class DailyReportFactory(object):
    ...
    def check_months(self, profile: MonitorProfile):
        ...
        if (
            profile.months.filter(name=f"{self.specify_day.month}月")
            or profile.always_display
        ):
    ...
    def get_data(
        ...
        # 篩選出前一週資料
        last_week_df = df[
            (pd.to_datetime(df["date"]).dt.date >= self.last_week_start.date())
        ...
        if has_volume:
            ...
            if last_week_avg_volume > 0:
                self.result[product_name].update(
                    {
                        # U 欄為 '與前一週比較(%)' 交易量欄位
                        f"U{row}": (this_week_avg_volume - last_week_avg_volume)
                        / last_week_avg_volume
                        * 100
                    }
                )
    ...
    def report(self):
        ...
        monitor_list = list(
            MonitorProfile.objects.filter(watchlist=watchlist, row__isnull=False)
        )
        ...
        self._extract_data(
            ...
            Fruit,
            50063,
            Source.objects.filter(id__in=[20001, 20002]),
            self.specify_day,
        )
        ...
        self._extract_data(
            ...
            Fruit,
            59019,
            Source.objects.filter(id__in=range(10030, 20001)),
            self.specify_day,
        )

        # 2020/4/16 主管會報陳副主委要求花卉品項,農糧署建議新增香水百合 FS
        # (30001, 臺北花市), (30002, 臺中市場), (30003, 彰化市場), (30004, 臺南市場), (30005, 高雄市場)
        self._extract_data(
            110,
            Flower,
            60068,
            Source.objects.filter(id__in=[30001, 30002, 30003, 30004, 30005]),
            self.specify_day,
        )
    ...
    def _extract_data(self, row, model, product_id, sources=None, date=None):
        ...
        if product_id in HIDE_IF_EMPTY_PRODUCTS:
            ...
            # 抓取當週資料
            qs_vis = DailyTran.objects.filter(
                product=product,
                date__gte=self.this_week_start.date(),
                date__lte=self.this_week_end.date(),
            )
            if sources:
                qs_vis = qs_vis.filter(source__in=sources)

            # 當週完全沒有紀錄則不顯示（直接 return，不加入 row_visible）
            if not qs_vis.exists():
                return
        ...
        if date:
            query_set = DailyTran.objects.filter(
                product=product, date__year=date.year - 1, date__month=date.month
            )
    ...
    def roc_date_format(date, sep="."):
        return sep.join(
            [str(date.year - 1911), str(date.month).zfill(2), str(date.day).zfill(2)]
        )
    ...
    def get_sheet(self):
        ...
        sheet.cell(row=137, column=1).value = sheet.cell(
            row=137, column=1
        ).value.replace("本會", "本部")
        sheet.cell(row=138, column=1).value = sheet.cell(
            row=138, column=1
        ).value.replace("本會", "本部")
        ...
        desc_2_text = (
            f"4.{desc_join}交易量價(東勢果菜市場價格)－農產品行情報導，本部農糧署。"
        )
...
class QueryString:
    ...
    def add_where_by_product_in(self, products: List[AbstractProduct], has_and=True):
        ...
        q = self.add_and_keyword(
            f'"dailytrans_dailytran"."product_id" IN %({key})s ', has_and
        )
        self.base_query += q
    ...
    def add_where_by_product_id(self, product_id: int, has_and=True):
        key = "product_id"
        q = self.add_and_keyword(
            f'"dailytrans_dailytran"."product_id" = %({key})s ', has_and
        )
        self.base_query += q
    ...
    def add_where_by_source_in(self, monitor: MonitorProfile, has_and=True):
        ...
        key = "source_list"
        q = self.add_and_keyword(
            f'"dailytrans_dailytran"."source_id" IN %({key})s ', has_and
        )
        self.base_query += q
    ...
    def add_where_by_last_year_and_month(self, dt: datetime.date, has_and=True):
        ...
        start_date = datetime.datetime.strptime(
            f"{last_year}-{dt.month}-01", "%Y-%m-%d"
        )
        end_date = datetime.datetime.strptime(
            f"{last_year}-{dt.month}-{days}", "%Y-%m-%d"
        )
...
class DailyTranHandler:
    ...
    def has_volume(self) -> bool:
        ...
        return (not self.df.empty) and (
            self.df["volume"].notna().sum() / self.df["avg_price"].count() > 0.8
        )
    ...
    def has_weight(self) -> bool:
        ...
        return (not self.df.empty) and (
            self.df["avg_weight"].notna().sum() / self.df["avg_price"].count() > 0.8
        )
    ...
    def fulfilled_df(self) -> pd.DataFrame:
        ...
        df = (
            self.df.query("volume > 0 and avg_weight > 0")
        ...
        df = (
            df.assign(vol_for_calculation=lambda x: x["volume"].fillna(1))
            .assign(wt_for_calculation=lambda x: x["avg_weight"].fillna(1))
            .assign(
                avg_price=lambda x: x["avg_price"]
                * x["wt_for_calculation"]
                * x["vol_for_calculation"]
            )
            .assign(
                wt_for_calculation=lambda x: x["wt_for_calculation"]
                * x["vol_for_calculation"]
            )
        )
        ...
        return (
            df.assign(source_id=lambda x: x["source_id"].fillna(1))
    ...
    def df_with_group_by_date(self) -> pd.DataFrame:
        ...
        df = (
            self.fulfilled_df.groupby(["date", "source_id"])
            .sum()
            ...
            .assign(
                avg_weight=lambda x: x["wt_for_calculation"] / x["vol_for_calculation"]
            )
            .reset_index()
            .sort_values("date")
            .rename(columns={"volume": "sum_volume", "avg_weight": "avg_avg_weight"})
            .assign(
                sum_volume=lambda x: np.where(
                    self.has_volume, x["vol_for_calculation"], x["num_of_source"]
                )
            )
            .assign(
                avg_avg_weight=lambda x: np.where(
                    self.has_weight, x["avg_avg_weight"], 1
                )
            )
        )
...
class ExtraItem:
    EXTRA_ITEMS = [
        ...
        {"product_id": 50063, "row": 73, "sources_id": [20001, 20002]},
        {
            "product_id": 59019,
            "row": 72,
    ...
    def __init__(
        self, product_id: int, row: int, sources_id: Optional[List[int]] = None
    ):
    ...
    def get_extra_monitors(cls) -> List[MonitorProfile]:
        ...
        for d in cls.EXTRA_ITEMS:
            ...
            monitor = MonitorProfile(
                product=product, watchlist=watchlist, type=product.type, row=item.row
            )
...
class ExcelHandler:
    ...
    def __init__(
        self,
        factory: "SimplifyDailyReportFactory",
        output_dir=settings.BASE_DIR("apps/dailytrans/reports"),
    ):
    ...
    def __set_text_of_date_range_and_avg_price(self):
        ...
        last_week_range_text = (
            f"{dt_start.month}/{dt_start.day}~{dt_end.month}/{dt_end.day}"
        )
            ...
            for product_name, dict_values in self.factory.result.items():
                for cell_name, avg_price in dict_values.items():
                    ...
                    if self.get_cell_format(cell_name):
                        self.__sheet[cell_name].number_format = self.get_cell_format(
                            cell_name
                        )
    ...
    def __set_other_desc(self):
        for i in self.__LIST_OTHER_DESC:
            self.__sheet.cell(
                row=self.desc_row_num, column=1
            ).value = f"{self.desc_list_num}.{i}"
    ...
    def get_cell_format(self, cell_name: str):
        ...
        return (
            self.__CELL_FORMAT.get(f"{alph1}{alph2}")
            if alph2.isalpha()
            else self.__CELL_FORMAT.get(alph1)
        )
...
class SimplifyDailyReportFactory:
    ...
    def show_monitor(self) -> bool:
        ...
        return (
            self.monitor.months.filter(name=f"{self.specify_day.month}月").exists()
            or self.monitor.always_display
        )
    ...
    def extend_query_str(self):
        ...
        if self.monitor.product.id == 50182:
            ...
            # 7, 8 月只抓新興梨 50185
                ...
                self.query.add_where_by_product_in(
                    self.monitor.product_list(), has_and=False
                )
    ...
    def set_this_week_data(self):
        ...
        query = self.query.add_where_by_date_between(
            self.last_week_start, self.this_week_end
        ).build()
        ...
        for named_tuple in self.two_weeks_handler.df_with_group_by_date.itertuples():
            ...
            # 設定最近一週的交易量
            if (
                self.two_weeks_handler.has_volume
                and named_tuple.date >= self.this_week_start.date()
            ):
                self.result[self.monitor.product.name].update(
                    {
    ...
    def set_avg_price_values(self):
        ...
        if last_week_avg_price > 0:
            self.result[self.monitor.product.name].update(
                {
                    f"L{self.monitor.row}": (this_week_avg_price - last_week_avg_price)
                    / last_week_avg_price
                    * 100
                }
            )
        ...
        self.result[self.monitor.product.name].update(
            {
                f"H{self.monitor.row}": this_week_avg_price,
                f"W{self.monitor.row}": last_week_avg_price,
            }
        )
    ...
    def set_volume_values(self):
        ...
        if self.two_weeks_handler.has_volume:
            ...
            # 當週交易量欄位(T)
            self.result[self.monitor.product.name].update(
                {f"T{self.monitor.row}": this_week_avg_volume}
            )
                ...
                self.result[self.monitor.product.name].update(
                    {
                        ...
                        f"U{self.monitor.row}": (
                            this_week_avg_volume - last_week_avg_volume
                        )
                        / last_week_avg_volume
                        * 100
                    }
    ...
    def set_monitor_price(self):
        ...
        if self.monitor.price:
            self.result[self.monitor.product.name].update(
                {f"F{self.monitor.row}": self.monitor.price}
            )
    ...
    def set_same_month_of_last_year_value(self):
        ...
        query = (
            self.query.load().add_where_by_last_year_and_month(self.specify_day).build()
        )
        ...
        if last_year_avg_price > 0:
            self.result[self.monitor.product.name].update(
                {f"G{self.monitor.row}": last_year_avg_price}
            )
    ...
    def update_ram_data(self):
        ...
        daily_trans = DailyTran.objects.filter_by_date_lte(
            days=this_week_date,
            products=self.monitor.product_list(),
            sources=self.monitor.sources(),
        )
        ...
        for i, d in enumerate(daily_trans):
            ...
            value = (
                d.avg_price if i == 0 or d.date == date else d.date.strftime("(%m/%d)")
            )
    ...
    def update_cattle_data(self):
        ...
        this_week_trans = DailyTran.objects.filter_by_date_lte(
            this_week_date, self.monitor.product_list()
        )
        this_week_avg_price = self.get_simple_avg_price(this_week_trans)
        last_week_trans = DailyTran.objects.filter_by_date_lte(
            last_week_date, self.monitor.product_list()
        )
